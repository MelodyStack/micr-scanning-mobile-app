/**
 * Turn a recognised MICR string into the fields the backend expects, gated on
 * the ABA checksum. A failed checksum means reject and retry, never return
 * garbage to the backend. Spec section 8.
 */
...
/**
 * Shortest account number treated as a real read: a plausibility floor against
 * truncation, which nothing else notices because the checksum covers only the
 * routing number. One cheque came back with `5` in place of `586033512335`. The
 * 14 reference cheques run 8 to 12 digits.
 */
const MIN_ACCOUNT_DIGITS = 6;

/**
 * Parse a substituted MICR line (T/A/O/D plus digits). Handles both layouts,
 * which is the trap spec section 8 calls out: splitting by position rather than
 * by "the digits before the first O" is what stops account numbers coming out
 * with the cheque number glued on.
 *
...
export function parseMicr(raw: string): ParseResult {
  ...
  // The second half of the truncation guard: a digit run that stops at the end
  // of the line was never terminated, so there is no evidence the account ended
  // there rather than the reading did. chk007 came back as `...T5728596` for a
  // true `...T572859650O`, checksum-valid and wrong by two digits.
  ...
  // The dash is a separator, not a digit. Kept in `raw` for traceability and
  // dropped from the value the backend receives, as the web app does.
